...
func TestParseRelaysURLs(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			relayURL: fmt.Sprintf("http://%s@foo.com?id=foo&boost=1", publicKey.String()),

			expectedURI:       "http://foo.com?id=foo&boost=1",
			expectedPublicKey: publicKey.String(),
			expectedURL:       fmt.Sprintf("http://%s@foo.com?id=foo&boost=1", publicKey.String()),
		},
		{
			name:     "Relay URL with query arg",
			relayURL: fmt.Sprintf("http://%s@foo.com?id=foo&boost=1", publicKey.String()),
			...
			expectedURI:       "http://foo.com/eth/foo?id=foo&boost=1",
			expectedPublicKey: publicKey.String(),
			expectedURL:       fmt.Sprintf("http://%s@foo.com?id=foo&boost=1", publicKey.String()),
		},
	}
