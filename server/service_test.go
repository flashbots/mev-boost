...
func blindedBlockToExecutionPayload(signedBlindedBeaconBlock *consensusapi.SignedBlindedBeaconBlock) *bellatrix.ExecutionPayload {
...
var payloadRegisterValidator = consensusapi.SignedValidatorRegistration{
	Message: &consensusapi.ValidatorRegistration{
...
func TestRegisterValidator(t *testing.T) {
	...
	reg := consensusapi.SignedValidatorRegistration{
		Message: &consensusapi.ValidatorRegistration{
			FeeRecipient: _HexToAddress("0xdb65fEd33dc262Fe09D9a2Ba8F80b329BA25f941"),
			Timestamp:    time.Unix(1234356, 0),
			Pubkey: _HexToPubkey(
	...
	payload := []consensusapi.SignedValidatorRegistration{reg}
...
func TestGetHeader(t *testing.T) {
	...
	t.Run("Use header with highest value", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, uint256.NewInt(12347), resp.Data.Message.Value)
	})
	...
	t.Run("Invalid relay signature", func(t *testing.T) {
		...
		backend.relays[0].GetHeaderResponse.Data.Signature = phase0.BLSSignature{}
...
func TestGetPayload(t *testing.T) {
	...
	payload := consensusapi.SignedBlindedBeaconBlock{
		...
		Message: &consensusapi.BlindedBeaconBlock{
			Slot:          1,
			ProposerIndex: 1,
			...
			Body: &consensusapi.BlindedBeaconBlockBody{
				...
				SyncAggregate: &altair.SyncAggregate{
					SyncCommitteeBits:      make([]byte, 512),
					SyncCommitteeSignature: phase0.BLSSignature{0xa4},
				},
...
func TestGetPayloadWithTestdata(t *testing.T) {
	...
	for _, fn := range testPayloadsFiles {
		t.Run(fn, func(t *testing.T) {
			...
			signedBlindedBeaconBlock := new(consensusapi.SignedBlindedBeaconBlock)
			require.NoError(t, DecodeJSON(jsonFile, &signedBlindedBeaconBlock))

			backend := newTestBackend(t, 1, time.Second)
			mockResp := api.VersionedExecutionPayload{
				Version:   consensusspec.DataVersionBellatrix,
				Bellatrix: blindedBlockToExecutionPayload(signedBlindedBeaconBlock),
			}
